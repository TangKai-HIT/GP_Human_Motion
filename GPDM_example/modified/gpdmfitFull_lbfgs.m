function [X, theta, thetap, weights] = gpdmfitFull_lbfgs(X, Y, weights,...
    segments, theta, thetap, options, extIters, modelType, missing)
%%gpdmfitFull_lbfgs: simplified version of the original file gpdmfitFull,
%%use L-BFGS instead of SCG

PRINT_INFO = 0; %print info every external loop
PLOT_L = 1; %plot negative log-likelihood every 10 L-BFGS loop
global FIX_HP
global LEARN_SCALE
global MARGINAL_DW
global W_VARIANCE
...
for iters = 1:extIters
    ...
    fminoptions = optimoptions('fminunc','MaxIterations', opt14, 'SpecifyObjectiveGradient',true, 'Algorithm', 'quasi-newton');
   
    func=@(t) objfunc(t, Y, weights, segments, modelType, missing);
    [params, options(8)] = fminunc(func, params, fminoptions);
...
%         Y, weights, segments, modelType, missing);
    ...
    energyLog_lbfgs = [energyLog_lbfgs, options(8)];
    
    w = weights;
    thetad = thetap; 
    
    if (iters==extIters) 
        save log_lbfgs energyLog_lbfgs;  
    end

    if (size(energyLog_lbfgs, 2) > 1)
        if ((energyLog_lbfgs(end-1) - energyLog_lbfgs(end)) < options(3) && ...
                changeX < options(2) && changeW < options(2) && ...
                changeTheta < options(2))
            changeX
            changeW
            changeTheta
            size(flog,2)
