function gpdmvisualise_m1(ax, X, Y, segments, invK, theta, invKd, thetad,  modelType, plotType, W, oldaxis)

% GPLVMVISUALISE Visualise the manifold.
side_max = max(max(X));
side_min = min(min(X));
if ~exist('oldaxis','var') 
...
if (plotType == 0) % Reconstruction Variance Map (log)
    ...
    colorbarLabel = "Reconstruction Log-Variance";
elseif (plotType == 1) %Dynamic Latent Space Prediction Variance Map (log)
    ...
    colorbarLabel = "Log Dynamic Prediction Log-Variance";
elseif (plotType == 2) %Dynamic Latent Space Prediction Variance + Reconstruction Variance (log)
    [Xin, Xout] = priorIO(X, segments, modelType);
    ...
    colorbarLabel = "Reconstruction+Dynamic Prediction Log-Variance";
end
...
h = vol3d('cdata',Zi, 'Parent',ax, 'texture','3D'); 
mask = segments-1;
mask(find(mask == 0)) = [];
%SX = X;
% for n = 1:q
...
if size(S,1) > 1
    ...
    quiver3(ax, S(:,1), S(:,2), S(:,3), dS(:,1), dS(:,2), dS(:,3), 0, 'b');
end

plot3(ax, S(:,1), S(:,2), S(:,3), 'bo');
